...
- Modify: `sra.py` — `questions T [--section S] [--status ...]`, `add-questions T --section S (--from-file F | --question Q ...) [--round N] [--origin P]`, `mark-answered T --question-hash H --sources IDS`, `record-attempt T --question-hash H ...` (see Task 9.2's plan decision: §20 defines `record_attempt` but §19's table has no command reaching it; this subcommand is the addition. Prints `{"hash", "attempts", "status"}` per hash; unknown hash exits 1)
...
Skill flow per round r ≤ R (§14): (1) select `status: open` questions (`sra.py questions T --status open`); (2) `batch_questions` → dispatch one `sra-researcher` per batch, ≤ `MAX_PARALLEL_AGENTS` in flight, successive waves for larger sets; (3) for each dispatched question that returned no citable evidence, the skill records the empty attempt through the driver — never by hand-editing the ledger.

**Plan decision (spec gap):** §20 defines `record_attempt` as a library function but §19's command table lists no command that reaches it, and a skill may not do bookkeeping itself (§3). Resolve by adding one subcommand, `sra.py record-attempt T --question-hash H [--question-hash H ...]`, which calls `record_attempt` per hash and prints `{"hash": "<h>", "attempts": N, "status": "open|deferred"}` per entry. This is an addition to §19's table, not a contradiction of it; document it in `CLAUDE.md`'s command table. Implement the subcommand in Task 8.1 alongside the other ledger commands (add its tests there: increments attempts, flips to `deferred` exactly at `MAX_ATTEMPTS`, unknown hash exits 1); (4) harvest barrier: `sra.py fetch-urls T`; (5) one synthesizer subagent per active section: reads new answers + `.urls.json` maps, writes wiki claims citing bronze ids only (never answer files), proposes follow-ups via `add-questions --origin synthesizer`, closes questions via `mark-answered --sources <bronze-ids>`, drops out-of-scope ones; (6) stop section when no material new questions; hard stop after R rounds; remaining questions stay `open`, listed under "Open questions" in the wiki page.
